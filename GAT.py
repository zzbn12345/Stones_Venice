...
def initialization():
    ...
    model = GAT_L(in_channels=data.x.shape[-1], hidden_channels = args.hidden_channels, 
            out_channels = data.y.shape[-1], dropout = args.dropout_p, heads=args.num_heads).to(device)
...
def Hypersearch(hyperdict, current_best, verbose):
    ...
    search_bar = tqdm(desc='hyper_searching', 
                              total=len(s_dropout)*len(s_num_heads)*len(s_hidden_channels)*len(s_lr))
