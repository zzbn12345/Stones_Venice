...
def test(model, loader):
    ...
    for batch in tqdm(loader):
        ...
        out_att = out[:,:9]
        out_val = out[:,9:]
        type_node = batch.node_type[:batch_size].nonzero().squeeze()
        ...
        loss_2 = F.cross_entropy(out_val[type_node], y_val[type_node])
        #loss_3 = loss_1 + loss_2
        
        acc_1_t = compute_1_accuracy(y_att[:batch_size], out_att)
        acc_1_val = compute_1_accuracy(y_val[type_node], out_val[type_node])
        acc_k_t = compute_k_accuracy(y_val[type_node], out_val[type_node], args.k)
        jac_k_t = compute_jaccard_index(y_val[type_node], F.softmax(out_val[type_node],dim=-1), args.k)
        ...
        running_1_val += float(acc_1_val) * batch_size
        running_k_acc += float(acc_k_t) * batch_size
        running_k_jac += float(jac_k_t) * batch_size

    return running_loss_1/total_examples, running_loss_2/total_examples, running_1_acc/ total_examples, running_k_acc/ total_examples, running_k_jac/ total_examples, running_1_val/total_examples
...
def initialization():
    ...
    model = HGT_L(data.metadata(), hidden_channels=args.hidden_channels, out_channels=data.y_dict['all'].shape[-1],
                  num_layers=args.num_layers, num_heads = args.num_heads, group=args.group).to(device)
...
def Hypersearch(hyperdict, current_best, verbose):
    ...
    s_num_layers = [2, 3, 5]
    s_group = ['sum', 'mean']
    s_hidden_channels = [32, 64, 128, 256]
    #s_l2 = [0, 1e-5, 2e-4]
    #s_batch_size = [32]
    s_lr = [0.01, 0.001, 0.0005, 0.0001]
    
    search_bar = tqdm(desc='hyper_searching', 
                              total=len(s_num_heads)*len(s_num_layers)*len(s_group)*len(s_hidden_channels)*len(s_lr))
    
    i=0
    
    for hd in s_num_heads:
