...
def initialization():
    ...
    model = GCN2_L(in_channels=data.x.shape[-1], hidden_channels = args.hidden_channels, 
            out_channels = data.y.shape[-1], dropout = args.dropout_p, num_layers = args.num_layers, alpha=0.5, 
...
def Hypersearch(hyperdict, current_best, verbose):
    ...
    s_num_layers = [3, 6, 9]
    s_hidden_channels = [128, 256, 512, 1024, 2048]
    #s_l2 = [0, 1e-5, 2e-4]
    #s_batch_size = [32]
    s_lr = [0.01, 0.001, 0.0001]
...
def main():
    ...
    for sd in s_seed:
        ...
        #if 'best_config.csv' in [files for root, dirs, files in os.walk(args.save_dir)][0]:
        #        with open(args.save_dir+'hyperdict.p', 'rb') as fp:
        #            hyperdict = pickle.load(fp)
        #else:
        current_best = {}
